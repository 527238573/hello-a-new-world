...
local fs = lovefs(eui.home)

local textinput_img = love.graphics.newImage("suit/assets/textback.png")
...
local function fileDialog(isopen)
  ...
  local s_close = eui.closeBtn(closeBtn_Opt,dlg.x+469,dlg.y+3)
  ...
  suit.List(filelist,function(num,x,y,w,h)
      ...
      local state = eui.fileButton(filelist.list[num], x,y,w,h,curSelect==num)
  ...
  if(s_confirm.hit) then
    if isopen then
      if curFileName~= "" then
        mapfile.openFile(curFileName,fs.current..fs.sep)
        eui.popwindow = nil
        curFileName =""
        curSelect =nil
      end
      ...
      if textinput_info.text ~=""then 
        mapfile.saveFile(textinput_info.text,fs.current..fs.sep)
        eui.popwindow = nil
        curFileName =""
        curSelect =nil
        textinput_info.text = ""
...
function eui.openFileDialog()
  fileDialog(true)
end

function eui.saveFileDialog()
  fileDialog(false)
end
