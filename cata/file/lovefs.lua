...
function filesystem:cd(dir)
	local current, tDirs, tFiles, tAll = self:ls(dir)
	if current then
...
function filesystem:exists(path)
	...
	local dirpath = self:absPath(path:match('(.*'..self.sep..')'))
	local name = path:match('[^'..self.sep..']+$')
	--ext = name:match('[^.]+$')
	local dir, dirs, files, all = self:ls(dirpath)
	if dir then
...
function filesystem:isFile(path)
	local exists, _, isFile = self:exists(path)
...
return function (dir)
	...
	if not temp:cd(dir) then
		if not temp:cd(temp.home) then
			if temp.win then temp:cd('c:'..temp.sep)
			else temp:cd(temp.sep) end
		end
	end
	temp:updDrives()
	return temp
