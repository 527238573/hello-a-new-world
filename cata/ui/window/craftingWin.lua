  ...
  for i=0,#sub_list do
    ...
    local function draw_sub_cat_btn()
      ...
      elseif opt.state=="active" then
        love.graphics.setColor(170,170,233)
        love.graphics.rectangle("fill",subx,suby,85,28,4,4)
...
function loadEntry()
  ...
  local text1 = {{22,22,22},tl("主要技能: ","Main skill: ")}
  ...
  if mainlevel>=requireLevel then
    text1[#text1+1] = {22,120,22}
    text1[#text1+1] = skillName
  else
    text1[#text1+1] = {120,22,22}
    text1[#text1+1] = skillName
  end
  ...
  if recipe.required_skills then
    --次要技能等级
    local text2 = {{22,22,22},tl("次要技能: ","Required skill: ")}
    for skill_id,require_level in pairs(recipe.required_skills) do
      ...
      if playerlevel>=require_level then
        text2[#text2+1] = {22,120,22}
        text2[#text2+1] = skillName
      else
        text2[#text2+1] = {120,22,22}
        text2[#text2+1] = skillName
      end
    end
  ...
  local text2p5 = {{22,22,22},tl("暗处制造: ","Dark craftable: ")}
  if recipe.flags["BLIND_EASY"] then
    text2p5[#text2p5+1] = {22,22,22}
  ...
  length = length+ Rtext:getHeight()+3
  local text4 = {{22,22,22},tl("需要工具: ","Tools required: ")}
  local toolLength = 0
  curEntry.qualities = {}
  ...
  Rtext:add({{22,22,22},tl("需要材料:","Components required:")},0,length)
  length = length+ Rtext:getHeight()+3
  ...
  if recipe.byproducts then
    Rtext:add({{22,22,22},tl("附加产物:","Byproducts:")},0,length)
    length = length+ Rtext:getHeight()+3
...
local function one_quality(qua,x,y)
  ...
  local function drawQText()
    if  qua.meet then
      love.graphics.setColor(22,120,22)
    else
      love.graphics.setColor(120,22,22)
    end
    love.graphics.setFont(c.font_c16)
