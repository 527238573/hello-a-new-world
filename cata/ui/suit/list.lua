...
return function(core, info,itemFunc, ...)
  ...
  local rectstate = core:ScrollRect(info,info.scrollrect_opt,x,y,w-17,h)
  core:registerHitbox(opt,opt.id, x,y,w-17,h) -- 底板
  local itemstates ={
		id = opt.id,
  ...
  for i =startnum,startnum+ todraw_num do
    local newstate = itemFunc(i,x,info.y+(i-1)*singleh,w-17,singleh)
